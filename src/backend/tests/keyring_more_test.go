...
func TestRecordSuccessKeepsPermanentDisableState(t *testing.T) {
	cfg := config.New(store.New(t.TempDir()))
	if err := cfg.SaveProvider(config.Provider{
		Name: "gemini",
		Type: config.Gemini,
		Keys: []config.Key{
			{Value: "k1", DisabledUntil: config.KeyPermanentlyDisabled, ConsecutiveFails: 4},
		},
	}); err != nil {
		t.Fatalf("保存提供商失败: %v", err)
	}
	...
	provider, _ := cfg.Provider("gemini")
	if provider.Keys[0].DisabledUntil != config.KeyPermanentlyDisabled {
		t.Fatalf("期望手动永久禁用状态保持不变，实际是 %+v", provider.Keys[0])
	}
