/* ---------- api ---------- */
    ...
    async function loadStatusOverview() {
      try {
        const response = await fetchStatusOverviewSnapshot();
    ...
    async function loginAdmin(adminKey) {
      const response = await sendAdminLoginRequest(adminKey);
      if (!response.ok) {
    ...
    async function loadAdminOverview() {
      const response = await fetchAdminOverviewSnapshot();
    ...
    async function saveProvider(form, statusElement) {
      ...
      const response = await sendSaveProviderRequest(payload);
    ...
    async function saveGlobalConfig() {
      ...
      const response = await sendSaveGlobalConfigRequest(payload);
    ...
    async function importKeys(form, statusElement) {
      ...
      const response = await sendImportKeysRequest(provider, parsedKeys);
    ...
    async function deleteProvider(name) {
      ...
      const response = await sendDeleteProviderRequest(name);
      if (!response.ok) {
    ...
    async function deleteKey(provider, key) {
      ...
      const response = await sendDeleteKeyRequest(provider, key);
      if (!response.ok) {
        throw new Error(await parseErrorMessage(response, t("error.delete")));
      }
    ...
    async function clearProviderCache(provider) {
      ...
      const response = await sendClearProviderCacheRequest(provider);
      if (!response.ok) {
    ...
    async function applyBulkKeyAction(providerName, actionName) {
      ...
      const response = await sendBulkKeyActionRequest(providerName, buildBulkKeyActionRequest(providerName, actionName, selectedKeys));
      if (!response.ok) {
        throw new Error(await parseErrorMessage(response, t("error.action")));
      }
