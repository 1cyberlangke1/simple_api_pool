...
/* ---------- core ---------- */
    ...
    async function loadAdminOverview() {
      ...
      if (response.status === 401) {
        ...
        renderAdminWorkspaceProviders();
        if (route === "admin") {
          setMessage(refs.loginStatus, t("admin.sessionExpiredKeepDrafts"), "error");
        }
        return;
      }
    ...
    async function deleteProvider(name) {
      if (!confirmAction(t("admin.confirmDeleteProvider", { name }))) {
        return;
      }
    ...
    async function deleteKey(provider, key) {
      if (!confirmAction(t("admin.confirmDeleteKey", { provider }))) {
        return;
      }
    ...
    async function clearProviderCache(provider) {
      if (!confirmAction(t("admin.confirmClearCache", { provider }))) {
        return;
      }
