...
func (ah *Handler) authorizeAdminRequest(w http.ResponseWriter, r *http.Request) bool {
	...
	if auth.CheckAdminAuthorizationHeader(r, ah.cfg) {
		if ah.limiter != nil {
			ah.limiter.RecordSuccess(r.RemoteAddr)
		}
		return true
	}
	if auth.CheckAdminSession(r, ah.cfg) {
...
func (ah *Handler) handleProviders(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
		httpapi.WriteJSONResponse(w, http.StatusOK, buildAdminProviderSnapshots(ah.cfg.Providers()))
	case http.MethodPost:
		...
		httpapi.WriteJSONResponse(w, http.StatusCreated, buildAdminProviderSnapshot(p))
	default:
...
func (ah *Handler) handleSingleProvider(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodDelete:
		...
		if ah.cache != nil {
			if err := ah.cache.ClearProvider(name); err != nil {
				httpapi.WriteErrorResponse(w, http.StatusInternalServerError, "删除提供商缓存失败")
				return
			}
		}
	...
	default:
		...
		httpapi.WriteJSONResponse(w, http.StatusOK, buildAdminProviderSnapshot(*p))
	}
}
...
func (ah *Handler) handleProviderKeys(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) < 1 {
		httpapi.WriteErrorResponse(w, http.StatusBadRequest, "请求路径无效")
		return
	}
	name := parts[0]
	...
	case http.MethodPost:
		...
		p, _ := ah.cfg.Provider(name)
		httpapi.WriteJSONResponse(w, http.StatusOK, buildAdminProviderSnapshot(*p).Keys)
	case http.MethodDelete:
		if len(parts) < 2 {
...
func (ah *Handler) handleConfig(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		...
		httpapi.WriteJSONResponse(w, http.StatusOK, GlobalConfigSnapshot{
...
func (ah *Handler) handleProviderKeyBulkAction(w http.ResponseWriter, r *http.Request) {
	...
	if body.Action != "enable" && body.Action != "disable" && body.Action != "delete" {
		httpapi.WriteErrorResponse(w, http.StatusBadRequest, "批量操作类型无效")
		return
	}
	...
	httpapi.WriteJSONResponse(w, http.StatusOK, buildAdminProviderSnapshot(*provider).Keys)
}

func (ah *Handler) resolveProviderKeyIdentifiers(providerName string, identifiers []string) []string {
...
func (ah *Handler) resolveProviderKeyIdentifier(providerName, identifier string) string {
	provider, _ := ah.cfg.Provider(providerName)
	if provider == nil {
		return ""
	}
